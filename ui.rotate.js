/*
 * uiRotate 0.1.0
 * http://github.com/cyan92128505/AngularRoulette
 *
 * Licensed under the MIT license
 */
(function (angular) {
    return angular.module('ui.rotate', [])
.service('uiRotateSupported', function () {
    var supportedCSS;
...
.service('uiRotateWilq32', ['uiRotateSupported', function (uiRotateSupported) {
            // Library agnostic interface
            var IE = uiRotateSupported.IE;
            var supportedCSS = uiRotateSupported.supportedCSS;
            var Wilq32 = {};
                    ...
                    if (events && this._eventObj) {
                        ...
                        if (this._parameters.bind) {
                            ...
                            for (var a in oldEvents) {
                                if (oldEvents.hasOwnProperty(a)) {
                                    angular.element(this._eventObj).off(a, oldEvents[a]);
                                }
                            }
                        }
                        ...
                        for (var a in events) {
                            if (events.hasOwnProperty(a)) {
                                angular.element(this._eventObj).on(a, events[a]);
                            }
...
.service('uiRotateCore', ['uiRotateWilq32', function (uiRotateWilq32) {
            return function (_rootElement) {
                var Wilq32 = uiRotateWilq32;
                var rootElement = angular.element(_rootElement);
                return {
                    rotate: function (parameters) {
                        if (rootElement.length === 0 || typeof parameters == "undefined") {
                            return;
                        }
                        ...
                        var returned = [];
                        for (var i = 0, i0 = rootElement.length; i < i0; i++) {
                            if (!rootElement[i].Wilq32 || !rootElement[i].Wilq32.PhotoEffect) {

                                var paramClone = angular.copy(parameters);
                                var newRotObject = new Wilq32.PhotoEffect(rootElement[i], paramClone)._rootObj;

                                returned.push(rootElement[i](newRotObject));
                            } else {
                                rootElement[i].Wilq32.PhotoEffect._handleRotation(parameters);
                            }
                        }
                        return returned;
                    },
                    getRotateAngle: function () {
                        ...
                        for (var i = 0, i0 = rootElement.length; i < i0; i++) {
                            if (rootElement[i].Wilq32 && rootElement[i].Wilq32.PhotoEffect) {
                                ret[i] = rootElement[i].Wilq32.PhotoEffect._angle;
                            }
                        }
                        return ret;
                    },
                    stopRotate: function () {
                        for (var i = 0, i0 = rootElement.length; i < i0; i++) {
                            if (rootElement[i].Wilq32 && rootElement[i].Wilq32.PhotoEffect) {
                                clearTimeout(rootElement[i].Wilq32.PhotoEffect._timer);
                            }
                        }
                    }
                }
